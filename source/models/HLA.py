...
vocab = np.load(
    os.path.join(_project_root, "data", "data_dict.npy"), allow_pickle=True
).item()
...
class Encoder_padding(nn.Module):
    ...
    def forward(self, enc_inputs):
        ...
        enc_pad = torch.zeros(enc_outputs.shape[0], hla_max_len, d_model).to(
            enc_outputs.device
        )
        # print(enc_outputs.size())
        enc_pad[:, : enc_outputs.shape[1], :] = enc_outputs
        enc_outputs = enc_pad
        enc_outputs = self.pos_emb_padding(enc_outputs.transpose(0, 1)).transpose(
            0, 1
...
class Cross_Attention(nn.Module):
    ...
    def forward(self, pep_inputs, HLA_inputs):
        ...
        dec_self_attn_pad_mask = (
            torch.LongTensor(np.zeros((pep_inputs.shape[0], hla_max_len, hla_max_len)))
            .bool()
            .to(device)
        )
        dec_self_attns = []
