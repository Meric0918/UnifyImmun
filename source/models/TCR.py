...
def data_load_tcr(type_="train", fold=None, batch_size=batch_size):
    ...
    loader = Data.DataLoader(
        MyDataSet_tcr(pep_inputs, hla_inputs, labels),
        batch_size,
        shuffle=False,
        num_workers=0,
        drop_last=True,
...
class Encoder_padding(nn.Module):
    ...
    def forward(self, enc_inputs):
        enc_outputs = self.src_emb(enc_inputs)
        enc_pad = torch.zeros(enc_outputs.shape[0], tcr_max_len, d_model)
...
class Cross_Attention(nn.Module):
    ...
    def forward(self, pep_inputs, HLA_inputs):
        ...
        dec_self_attn_pad_mask = (
            torch.LongTensor(np.zeros((pep_inputs.shape[0], tcr_max_len, tcr_max_len)))
            .bool()
            .to(device)
        )
        dec_self_attns = []
