...
Loads encoder_P from HLA_ESM2_2 and continues training.
...
print("Load Encoder from HLA_ESM2_2")

# Load encoder_P from HLA_ESM2_2
encoder_path = os.path.join(
    _project_root, "trained_model", "HLA_ESM2_2", "encoder_P_ESM2_2.pth"
)
if os.path.exists(encoder_path):
    model_tcr.encoder_P.load_state_dict(torch.load(encoder_path, map_location=device))
...
print("TCR ESM2 Phase 2 Train:")
path_all = os.path.join(_project_root, "trained_model", "TCR_ESM2_2")
save_path = os.path.join(path_all, "model_TCR_ESM2_2.pkl")
...
for epoch in range(1, epochs + 1):
    ...
    if val_improved:
        ...
        torch.save(model_tcr.eval().encoder_P.state_dict(), encoder_save_path)
        swanlab.log(
            {
...
print(f"TCR ESM2 Phase 2 Training Finished (best epoch: {epoch_best})")
...
if epoch_best >= 0:
    ...
    print("****TCR ESM2 Phase 2 Independent set:")
