"""
TCR binding model training Phase 2 with ESM2 embeddings.
Loads encoder_P from corresponding HLA_ESM2_2 fold and continues training.
...
def train_tcr(model, train_loader, fold, epoch, epochs, criterion, optimizer):
    train_time = 0
    model.train()
...
def valid_tcr(model, val_loader, fold, epoch, epochs, criterion):
    model.eval()
...
def train_single_fold(fold):
    """Train TCR model for a single fold, loading encoder_P from HLA_ESM2_2 fold."""
    random.seed(seed)
    np.random.seed(seed)
    ...
    swanlab.init(
        ...
        experiment_name=f"TCR_ESM2_2_fold_{fold}",
        config={
            "model": "TCR_ESM2_2",
            "embedding": "ESM2_650M",
            "epochs": epochs,
            ...
            "freeze_esm2": True,
            "fold": fold,
            "pretrained_encoder": f"HLA_ESM2_2_fold_{fold}",
            "early_stopping_patience": 10,
            "early_stopping_min_delta": 0.001,
        },
    )

    # Load encoder_P from corresponding HLA_ESM2_2 fold
    encoder_path_load = os.path.join(
        _project_root, "trained_model", "HLA_ESM2_2", f"encoder_P_ESM2_2_fold_{fold}.pth"
    )
    if os.path.exists(encoder_path_load):
        model.encoder_P.load_state_dict(torch.load(encoder_path_load, map_location=device))
        print(f"Loaded encoder_P from HLA_ESM2_2 fold-{fold}: {encoder_path_load}")
    else:
    ...
    train_data = pd.read_csv(
        os.path.join(_project_root, "data", "data_TCR", f"train_fold_{fold}.csv")
    ).dropna()
    val_data = pd.read_csv(
        os.path.join(_project_root, "data", "data_TCR", f"val_fold_{fold}.csv")
    ).dropna()
    print(
    ...
    # Paths for this fold
    path_all = os.path.join(_project_root, "trained_model", "TCR_ESM2_2")
    if not os.path.exists(path_all):
        os.makedirs(path_all)
    save_path = os.path.join(path_all, f"model_TCR_ESM2_2_fold_{fold}.pkl")
    ...
    if epoch_best >= 0:
        ...
        _, valid_performance, _, _ = valid_tcr(
            model_eval, val_loader, fold, epoch_best, epochs, criterion
        )
        _, independent_performance, _, _ = valid_tcr(
            model_eval, independent_loader, fold, epoch_best, epochs, criterion
        )
        _, covid_performance, _, _ = valid_tcr(
            model_eval, covid_loader, fold, epoch_best, epochs, criterion
        )
        _, triple_performance, _, _ = valid_tcr(
            model_eval, triple_loader, fold, epoch_best, epochs, criterion
        )
        ...
        fold_results = {
            ...
            "covid": covid_performance[:5],
            "triple": triple_performance[:5],
        }
        ...
        print("  Covid: roc_auc={:.4f}, acc={:.4f}, mcc={:.4f}, f1={:.4f}, aupr={:.4f}".format(*covid_performance[:5]))
        print("  Triple: roc_auc={:.4f}, acc={:.4f}, mcc={:.4f}, f1={:.4f}, aupr={:.4f}".format(*triple_performance[:5]))
...
def main():
    print("\n" + "#" * 60)
    print("# TCR_ESM2_2 5-Fold Cross-Validation Training (Phase 2)")
    print("#" * 60)
    print("# Loads encoder_P from corresponding HLA_ESM2_2 fold")
    ...
    for dataset_type in ["val", "independent", "covid", "triple"]:
    ...
    for fold in range(1, 6):
        ...
        if os.path.exists(save_path):
            print(f"  Fold-{fold}: model_TCR_ESM2_2_fold_{fold}.pkl, encoder_P_ESM2_2_fold_{fold}.pth")
