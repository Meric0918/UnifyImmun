...
def train_tcr(model, train_loader, fold, epoch, epochs):
    ...
    loss_list = []
    fgm = FGM(model)
    for train_pep_inputs, train_tcr_inputs, train_labels in tqdm(
        train_loader, colour="yellow"
    ):
        train_pep_inputs, train_tcr_inputs, train_labels = (
            train_pep_inputs.to(device),
            train_tcr_inputs.to(device),
        ...
        train_outputs2, train_dec_self_attns2 = model(
            train_pep_inputs, train_tcr_inputs
        )
    ...
    performance_train = performance(
        y_true_list, y_pred_list, y_pred_transfer_train_list
    )
...
def valid_tcr(model, val_loader, fold, epoch, epochs):
    ...
    with torch.no_grad():
        ...
        loss_val_list = []
        for val_pep_inputs, val_tcr_inputs, val_labels in tqdm(
            val_loader, colour="blue"
        ):
            val_pep_inputs, val_tcr_inputs, val_labels = (
                val_pep_inputs.to(device),
                val_tcr_inputs.to(device),
...
independent_loader_tcr = data_load_tcr(
...
(
    train_fold_performance_list_tcr,
    val_fold_performance_list_tcr,
    independent_fold_performance_list_tcr,
    covid_fold_performance_list_tcr,
    tripple_fold_performance_list_tcr,
) = [], [], [], [], []
(
...
for fold in range(1, 6):
    ...
    model_tcr.encoder_P.load_state_dict(
        torch.load(
            os.path.join(
                _project_root, "trained_model", "HLA_2", "encoder_P_{}.pth".format(fold)
            )
        )
    )
    ...
    train_data_tcr = pd.read_csv(
        os.path.join(
            _project_root, "data", "data_TCR", "train_fold_{}.csv".format(fold)
        )
    ).dropna()
    val_data_tcr = pd.read_csv(
        os.path.join(_project_root, "data", "data_TCR", "val_fold_{}.csv".format(fold))
    ).dropna()
    print(
        "Fold-{} Label: Train = {} | Val = {}".format(
            fold, Counter(train_data_tcr.label), Counter(val_data_tcr.label)
        )
    )
    print("TCR Train:")
    path_all_tcr = os.path.join(_project_root, "trained_model", "TCR_2")
    save_path_tcr = os.path.join(
        _project_root, "trained_model", "TCR_2", "model_TCR_fold{}.pkl".format(fold)
    )
    encoder_path_tcr = os.path.join(
        _project_root, "trained_model", "TCR_2", "encoder_P_{}.pth".format(fold)
    )
    ...
    for epoch in range(1, epochs + 1):
        (
            result_train_tcr,
            performance_train_tcr,
            time_train_ep_tcr,
            attention_score_tcr,
        ) = train_tcr(model_tcr, train_loader_tcr, fold, epoch, epochs)
        result_val_tcr, performance_val_tcr, attention_score_val_tcr = valid_tcr(
            model_tcr, val_loader_tcr, fold, epoch, epochs
        )
        ...
        swanlab.log(
            {
                "fold": fold,
                "epoch": epoch,
                "train_loss": float(f_mean(result_train_tcr[0])),
                "train_roc_auc": performance_train_tcr[0],
                "train_accuracy": performance_train_tcr[1],
                "train_mcc": performance_train_tcr[2],
                "train_f1": performance_train_tcr[3],
                "train_aupr": performance_train_tcr[4],
                "train_sensitivity": performance_train_tcr[5],
                "train_specificity": performance_train_tcr[6],
                "val_loss": float(f_mean(result_val_tcr[0])),
                "val_roc_auc": performance_val_tcr[0],
                "val_accuracy": performance_val_tcr[1],
                "val_mcc": performance_val_tcr[2],
                "val_f1": performance_val_tcr[3],
                "val_aupr": performance_val_tcr[4],
                "val_sensitivity": performance_val_tcr[5],
                "val_specificity": performance_val_tcr[6],
        ...
        if performance_avg > performance_best_tcr:
            ...
            print(
                "****Saving model: Best epoch = {} | metrics_Best_avg = {:.4f}".format(
                    epoch_best_tcr, performance_best_tcr
                )
            )
            ...
            swanlab.log(
                {
                    "best_epoch": epoch_best_tcr,
                    "best_performance_avg": performance_best_tcr,
                }
            )
    ...
    if epoch_best_tcr >= 0:
        ...
        (
            independent_result_tcr,
            independent_performance_tcr,
            independent_attention_tcr,
        ) = valid_tcr(
            model_eval_tcr, independent_loader_tcr, fold, epoch_best_tcr, epochs
        )
        covid_result_tcr, covid_performance_tcr, covid_attention_tcr = valid_tcr(
            model_eval_tcr, covid_loader_tcr, fold, epoch_best_tcr, epochs
        )
        tripple_result_tcr, tripple_performance_tcr, tripple_attention_tcr = valid_tcr(
            model_eval_tcr, tripple_loader_tcr, fold, epoch_best_tcr, epochs
        )
        ...
        swanlab.log(
            {
                f"fold_{fold}_independent_roc_auc": independent_performance_tcr[0],
                f"fold_{fold}_independent_accuracy": independent_performance_tcr[1],
                f"fold_{fold}_covid_roc_auc": covid_performance_tcr[0],
                f"fold_{fold}_covid_accuracy": covid_performance_tcr[1],
                f"fold_{fold}_triple_roc_auc": tripple_performance_tcr[0],
                f"fold_{fold}_triple_accuracy": tripple_performance_tcr[1],
            }
        )
