...
Loads encoder_P from corresponding HLA_ESM2 fold.
"""
...
def train_single_fold(fold):
    """Train TCR model for a single fold, loading encoder_P from HLA_ESM2 fold."""
    random.seed(seed)
    np.random.seed(seed)
    ...
    swanlab.init(
        ...
        experiment_name=f"TCR_ESM2_fold_{fold}",
        config={
            "model": "TCR_ESM2",
            "embedding": "ESM2_650M",
            "epochs": epochs,
            ...
            "freeze_esm2": True,
            "fold": fold,
            "pretrained_encoder": f"HLA_ESM2_fold_{fold}",
            "early_stopping_patience": 10,
            "early_stopping_min_delta": 0.001,
        },
    )

    # Load encoder_P from corresponding HLA_ESM2 fold
    encoder_path_load = os.path.join(
        _project_root, "trained_model", "HLA_ESM2", f"encoder_P_ESM2_fold_{fold}.pth"
    )
    if os.path.exists(encoder_path_load):
        model.encoder_P.load_state_dict(torch.load(encoder_path_load, map_location=device))
        print(f"Loaded encoder_P from HLA_ESM2 fold-{fold}: {encoder_path_load}")
    else:
    ...
    save_path = os.path.join(path_all, f"model_TCR_ESM2_fold_{fold}.pkl")
    encoder_path = os.path.join(path_all, f"encoder_P_ESM2_fold_{fold}.pth")
...
def main():
    print("\n" + "#" * 60)
    print("# TCR_ESM2 5-Fold Cross-Validation Training")
    print("#" * 60)
    print("# Loads encoder_P from corresponding HLA_ESM2 fold")
    ...
    for fold in range(1, 6):
        ...
        if os.path.exists(save_path):
            print(f"  Fold-{fold}: model_TCR_ESM2_fold_{fold}.pkl, encoder_P_ESM2_fold_{fold}.pth")
