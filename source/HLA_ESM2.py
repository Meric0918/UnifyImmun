...
# Only optimize trainable parameters (projection layers)
...
def train_HLA(model, train_loader, fold, epoch, epochs):
    ...
    for pep_inputs, hla_inputs, labels, pep_masks, hla_masks in tqdm(
        ...
        # Adversarial training on trainable parameters only
...
print("HLA Train with ESM2:")
path_all = os.path.join(_project_root, "trained_model", "HLA_ESM2")
save_path = os.path.join(path_all, "model_HLA_ESM2.pkl")
...
print(f"HLA ESM2 Training Finished (best epoch: {epoch_best})")
