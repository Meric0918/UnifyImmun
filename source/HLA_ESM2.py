"""
HLA binding model training with ESM2 embeddings.
...
def train_single_fold(fold):
    """Train HLA model for a single fold."""
    ...
    swanlab.init(
        ...
        experiment_name=f"HLA_ESM2_fold_{fold}",
        config={
            "model": "HLA_ESM2",
            "embedding": "ESM2_650M",
            "epochs": epochs,
    ...
    # Paths for this fold
    path_all = os.path.join(_project_root, "trained_model", "HLA_ESM2")
    save_path = os.path.join(path_all, f"model_HLA_ESM2_fold_{fold}.pkl")
...
def main():
    ...
    print("\n" + "#" * 60)
    print("# HLA_ESM2 5-Fold Cross-Validation Training")
    print("#" * 60)
    ...
    for fold in range(1, 6):
        ...
        if os.path.exists(save_path):
            print(f"  Fold-{fold}: model_HLA_ESM2_fold_{fold}.pkl, encoder_P_ESM2_fold_{fold}.pth")
